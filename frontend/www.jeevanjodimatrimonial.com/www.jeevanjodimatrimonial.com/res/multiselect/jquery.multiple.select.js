...
(function($) {
    ...
    MultipleSelect.prototype = {
        ...
        init: function() {
            ...
            html.push('<ul>');
            if (!this.options.MaxCheck && this.options.selectAll) {
                html.push(
                    '<li>',
                    '<label>',
            ...
            this.$drop.find('ul').css('width', '95%');
        ...
        update: function() {
            ...
            var dp = this.$drop;
            var t = true;
            if (this.options.MaxCheck && selects.length == this.options.MaxCheck) {
                $span.removeClass('placeholder').html("");
                var that = this,
                    texts = [],
                    values = [];
                this.$drop.find('input[name="selectItem"]').each(function() {
            ...
            if (t) {
                ...
                for (var i = 0; i < selects.length; i++) {
                    var btn = $("<span class='close'>" + $.trim(selects[i]) + " </span> ");
                    $span.append(btn);
                    $(btn).click(function() {
                        var txt = $(this).text();
                        //$(this).remove();
                        var s = $(dp).find("input[name='selectItem']:checked");
                        if (s.length > 0) {
                            for (var t = 0; t < s.length; t++) {
                                if ($(s[t]).val() === $.trim(txt)) {
                                    $(s[t]).removeAttr("checked");
                                    $($(s[t])).triggerHandler("change");
                                    $($(s[t])).triggerHandler("click");
                                    $(s[t]).removeAttr("checked");
                                }
                            }
                        }
        ...
        updateSelectAll: function() {
            ...
            var i = $(this.$choice).find("span:first").height();
            if (i < 30) i = 22; {
                $(this.$choice).height(i);
                if (i > 30) {
                    $(this.$choice).find("span:first").css('background', '');
                    $(this.$choice).css('background', '#fff');
                } else {
                    $(this.$choice).find("span:first").css('background', '');
                    $(this.$choice).css('background', '#fff');
                }
