...
const Chat = () => {
  ...
  const currentRoom = useMemo(
    () => rooms.find((r) => r._id.toString() === roomId),
    [rooms, roomId]
  );

  const getProfilePic = (participant) => {
    if (participant?.profileImage?.url) return participant.profileImage.url;
    return (
      participant?.profileImage || "https://via.placeholder.com/40x40?text=PP"
    );
  };
  ...
  const otherProfilePic = useMemo(
    () => getProfilePic(otherParticipant),
    [otherParticipant]
  );
  ...
  const handleNewMessage = useCallback(
    (msg) => {
      ...
      dispatch(
        setRooms(
          rooms.map((r) =>
            r._id.toString() === msg.roomId
              ? { ...r, lastMessage: msg, updatedAt: new Date(msg.createdAt) }
              : r
          )
        )
      );
  ...
  const fetchRooms = useCallback(async () => {
    try {
      const res = await axios.get(
        "https://jeevanshaadi.onrender.com/api/chat/rooms",
  ...
  const setRoom = useCallback(
    async (id) => {
      ...
      try {
        const res = await axios.get(
          `https://jeevanshaadi.onrender.com/api/chat/rooms/${id}/messages`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        if (res.data.success) {
  ...
  const confirmDeleteMessage = useCallback(async () => {
    ...
    try {
      await axios.delete(
        `https://jeevanshaadi.onrender.com/api/chat/rooms/${roomId}/messages/${messageToDelete}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="flex h-[90vh] bg-[#f0f2f5] overflow-hidden relative">
      ...
      <div
        className={`${
          !showSidebar || roomId ? "flex" : "hidden"
        } md:flex flex-1 flex-col bg-[#efeae2] transition-all duration-300`}
      >
        {roomId ? (
          <>
            {/* Header */}
              ...
              {messages.length === 0 ? (
                ...
                messages.map((msg) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`relative max-w-[85%] sm:max-w-[75%] md:max-w-[65%] px-4 py-2 rounded-xl shadow-lg transition-all ${
                          isOwn
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center max-w-md px-4">
              ...
              <h3 className="text-2xl sm:text-3xl font-light text-gray-800 mb-3">
                Welcome to Messages
              </h3>
